...
const chalk_1 = __importDefault(require("chalk")); // 문자열 색상용
/**
...
const stackWithMetaFormat = printf((info) => {
    ...
    if (method !== undefined)
        method = method;
    const ts = chalk_1.default.yellow(timestamp);
    const sc = chalk_1.default.red("statusCode:");
    const p = chalk_1.default.red("path:");
    const m = chalk_1.default.red("method:");
    return `${ts} | ${sc} ${statusCode ?? "-"} | ${p} '${path ?? "-"}' | ${m} ${method ?? "-"} | ${stack || message}`;
});
// 로거 인스턴스 생성
const logger = winston_1.default.createLogger({
    ...
    format: combine(timestamp({ format: "YYYY-MM-DD HH:mm:ss" }), errors({ stack: true }), stackWithMetaFormat),
    // 출력 대상
    transports: [
        // 개발용: 컬러링된 콘솔 출력
