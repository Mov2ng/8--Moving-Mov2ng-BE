...
if (!token || !channelId) {
    ...
    console.warn("⚠️ DISCORD_TOKEN 또는 CHANNEL_ID가 없어 Discord 봇을 비활성화합니다.");
}
else {
    ...
    const client = new discord_js_1.Client({
        ...
        intents: [
            discord_js_1.GatewayIntentBits.Guilds, // 서버(길드) 정보에 접근
            discord_js_1.GatewayIntentBits.GuildMessages, // 서버의 메시지 정보에 접근
        ],
    });
    // 스케줄러 설정용 함수 (기간 + 요일 + 시간)
    function scheduleThread({ hour, text, threadName }) {
        // cron 표현식: '분 시 일 월 요일'
        // '0 ${hour} * * 1-5' → 매주 월요일부터 금요일까지, 지정된 'hour'시 정각에 작업 실행
        node_cron_1.default.schedule(`0 ${hour} * * 1-5`, async () => {
            const now = new Date();
            ...
            if (!channel ||
                !(channel instanceof discord_js_1.TextChannel || channel instanceof discord_js_1.NewsChannel))
            ...
            try {
                ...
                console.log(`🧵 ${threadName} 스레드 생성 완료! (${kst.toLocaleString("ko-KR")})`);
            }
            catch (err) {
